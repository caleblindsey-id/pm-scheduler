...
import type { TechLeadStatus, UserRole } from '@/types/database'
...
type TabKey = 'pending' | 'approved' | 'earned' | 'paid' | 'closed' | 'payout'
...
function partitionByTab(leads: TechLeadWithJoins[], tab: TabKey): TechLeadWithJoins[] {
  switch (tab) {
    ...
    case 'closed':   return leads.filter(l => l.status === 'rejected' || l.status === 'cancelled')
    default:         return []
  }
}
...
export default function TechLeadsClient({ leads }: Props) {
  ...
  const filtered = useMemo(() => partitionByTab(leads, tab), [leads, tab])

  const counts = useMemo(() => {
    const c: Record<TabKey, number> = { pending: 0, approved: 0, earned: 0, paid: 0, closed: 0, payout: 0 }
                ...
                return (
                  <tr key={lead.id} className="align-top">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      ...
                      {lead.status === 'approved' && !lead.equipment_id && (
                        <div className="flex gap-2 justify-end">
                          <button
                            type="button"
