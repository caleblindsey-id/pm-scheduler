-- Migration 041: Parts Order Queue — expose vendor_item_code
-- PartRequest.vendor_item_code was added alongside product_number so the office
-- can order against the correct manufacturer/vendor SKU. Surface it on the
-- parts_order_queue view so it shows up (and is editable) on the Parts Queue page.

-- CREATE OR REPLACE VIEW cannot insert a new column in the middle of the
-- existing column list, so drop + recreate is required.
